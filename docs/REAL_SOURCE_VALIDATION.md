...
发布 CI 首轮发现两处仅在 Windows 显现的测试假设：案例权限测试硬编码 POSIX 0644，Unicode 源码语法检查未显式声明 UTF-8。保留失败运行，分别按平台权限语义检查、显式 UTF-8 传输修正后重跑三平台 CI；未跳过测试或删除 Unicode 输入，MCP/案例运行实现未因此改变。
